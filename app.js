...
function renderOverview() {
  ...
  const isCurrentMonth = State.year === todayYear && State.month === todayMonth;
  const isPastMonth    = State.year < todayYear || (State.year === todayYear && State.month < todayMonth);
  ...
  const balCard      = balEl ? balEl.closest('.balance-card') : null;

  // Remover badge de mês encerrado residual
  const oldBadge = document.getElementById('mes-encerrado-badge');
  if (oldBadge) oldBadge.remove();

  if (isPastMonth) {
    // ── Mês encerrado: usa apenas valores realizados, sem rotina de previsão ──
    if (saldoSection) saldoSection.style.display = 'none';
    if (balLabel) balLabel.textContent = 'Saldo Final';

    const enc = Finance.calcSaldoFinalMesEncerrado(State.year, State.month);
    balEl.textContent = fmtCurrency(enc.saldoFinal);
    balEl.className   = 'balance-amount ' + (enc.saldoFinal >= 0 ? 'positive' : 'negative');
    balSub.textContent = `Saldo anterior ${fmtCurrency(enc.saldoMesAnterior)} · Entradas ${fmtCurrency(enc.entradasRealizadas)} · Saídas ${fmtCurrency(enc.saidasRealizadas + enc.faturasPagas)}`;

    if (balCard) {
      const badge = document.createElement('div');
      badge.id = 'mes-encerrado-badge';
      badge.style.cssText = 'font-size:0.7rem;font-weight:700;text-transform:uppercase;letter-spacing:0.08em;color:var(--muted-fg);margin-bottom:0.5rem;';
      badge.textContent = '● Mês Encerrado';
      balCard.insertBefore(badge, balCard.firstChild);
    }

    // Ocultar fluxo previsto em meses encerrados
    const cfEl = document.getElementById('overview-cashflow');
    if (cfEl) { cfEl.style.display = 'none'; cfEl.innerHTML = ''; }
  } else {
    // ── Mês vigente ou futuro: comportamento original ──
...
function _preselectTxCategory(tx) {
  ...
  if (!sel) return;
  if (tx.type === 'income') {
    // Tenta preselecionar por ID (categorias customizadas) ou por nome (legado)
    if (tx.categoryId && [...sel.options].some(o => o.value === tx.categoryId)) {
...
function saveTxForm() {
  ...
  const catName = catOpt?.dataset.name || catSel.value || '';
  const payment   = document.getElementById('tx-payment').value;
...
function saveQuickForm() {
  ...
  const guessed = CatIntel.classify(parsed.description, 'expense');
  if (guessed) {
...
function saveCategoryForm() {
  ...
  if (_editCategoryId) {
    Categories.update(_editCategoryId, { name, icon, color, active, type, keywords: _categoryKeywordsDraft });
    showToast('Categoria atualizada', 'success');
  } else {
    Categories.create({ name, icon, color, type, keywords: _categoryKeywordsDraft });
