...
// Seed mínimo para novos usuários — apenas 5 categorias (3 de saída + 2 de entrada).
// Criado somente quando o usuário não possui nenhuma categoria (conta nova OU
// conta antiga migrada antes desta funcionalidade existir).
const DEFAULT_CATEGORIES_SEED = [
  { name: 'Carro',       icon: '🚗', color: '#3b82f6', type: 'expense', keywords: ['combustivel','gasolina','etanol','posto','ipva','licenciamento','revisao','mecanico','pneu','seguro do carro'] },
  { name: 'Alimentação', icon: '🍔', color: '#f97316', type: 'expense', keywords: ['lanche','hamburguer','pizza','salgado','padaria','restaurante','mcdonalds','burger king','ifood','comida','almoco','jantar','lanchonete','delivery'] },
  { name: 'Mercado',     icon: '🛒', color: '#22c55e', type: 'expense', keywords: ['mercado','atacadao','assai','extra','carrefour','tenda','spani','supermercado','hortifruti','feira'] },
  { name: 'Salário',     icon: '💰', color: '#059669', type: 'income',  keywords: ['salario','pagamento','holerite','vencimento'] },
  { name: 'Hora Extra',  icon: '⏰', color: '#eab308', type: 'income',  keywords: ['hora extra','adicional','horas extras'] },
];
...
const Categories = {
  ...
  update(id, data) {
    ...
    list[idx] = {
      ...
      ...('type' in data ? { type: data.type === 'income' ? 'income' : 'expense' } : {}),
...
const CatIntel = {
  ...
  // type (opcional): 'expense' ou 'income' — restringe a busca ao tipo informado.
  matchByKeywords(description, type) {
    ...
    let bestScore = 0;
    const cats = type
      ? Categories.active().filter(c => c.type === type)
      : Categories.active();
  ...
  // type (opcional): restringe a busca de palavras-chave ao tipo informado.
  classify(description, type) {
    return this.findLearned(description) || this.matchByKeywords(description, type);
  },
